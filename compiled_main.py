...
def train_and_save_model():
    ...
    ds_train_pretrain, ds_val_pretrain, ds_test_pretrain = process_data(
        ds_train, ds_val, ds_test, ds_info, batch_size=100, pretraining=True)
    ...
    labels = list(map(lambda x: x[1], tfds.as_numpy(ds_test)))



    ds_train_tsne, ds_val_tsne, ds_test_tsne = process_data(
        ds_train, ds_val, ds_test, ds_info, batch_size=5000, pretraining=False)

    actual_model = ptsne.tsne_model()
    tsne_coords_no_fine = actual_model.predict(ds_test_tsne)
    show_scatter(tsne_coords_no_fine, labels, "tsne_scatter_no_fine")
    ...
    show_scatter(tsne_coords, labels, "tsne_scatter")
